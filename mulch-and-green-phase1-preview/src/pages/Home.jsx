...
import { HARDSCAPE, SOFTSCAPE, GARDEN_DESIGN } from "@/data/landscapingData";
...
export default function Home() {
  ...
  return (
    <>
      ...
      {/* OUR MAIN SERVICES — Hardscape / Softscape / Garden Design */}
      <section className="py-24 md:py-32 bg-white">
        <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12">
          <div className="max-w-2xl mb-14 reveal">
            <span className="text-forest font-bold uppercase tracking-[0.22em] text-xs">Our Main Services</span>
            <h2 className="mt-3 font-display font-extrabold text-3xl sm:text-4xl lg:text-5xl text-[#1A1A1A] tracking-tight">
              Built on structure. Finished with life.
            </h2>
            <p className="mt-5 text-[#4A4A4A] text-lg leading-relaxed">
              Every project starts with one of three core services — explore how each one works, step by step.
            </p>
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-3 gap-7">
            {[
              { data: HARDSCAPE, to: "/landscaping/hardscape", badge: "bg-forest text-white" },
              { data: SOFTSCAPE, to: "/landscaping/softscape", badge: "bg-grass text-[#0F1B11]" },
              { data: GARDEN_DESIGN, to: "/landscaping/creative-plant-beds-garden-design", badge: "bg-earth text-white" },
            ].map(({ data, to, badge }) => (
              <div key={to} className="card-lift bg-white rounded-3xl border border-[#EFECD3] overflow-hidden reveal flex flex-col">
                <div className="img-zoom relative aspect-[4/3]">
                  <img
                    src={data.heroImage.full}
                    srcSet={`${data.heroImage.sm} 800w, ${data.heroImage.full} 1600w`}
                    sizes="(max-width: 1024px) 100vw, 33vw"
                    alt={`${data.name} in Windsor, Ontario`}
                    className="w-full h-full object-cover"
                    loading="lazy"
                    decoding="async"
                  />
                  <span className={`absolute top-4 left-4 px-3 py-1 rounded-full text-[11px] font-bold tracking-widest uppercase ${badge}`}>
                    {data.name.replace(" Services", "").replace("Creative Plant Beds & ", "")}
                  </span>
                </div>
                <div className="p-7 flex flex-col flex-1">
                  <h3 className="font-display font-bold text-xl text-[#1A1A1A]">{data.name}</h3>
                  <p className="mt-2 text-[15px] text-[#4A4A4A] leading-relaxed flex-1">{data.subtitle}</p>
                  <div className="mt-6 flex flex-wrap gap-2.5">
                    <Link
                      to={to}
                      data-testid={`mainservice-process-${data.slug}`}
                      className="inline-flex items-center gap-1.5 px-4 py-2.5 rounded-full border border-forest text-forest text-sm font-bold hover:bg-forest hover:text-white transition-colors"
                    >
                      View Process
                    </Link>
                    <Link
                      to="/contact"
                      data-testid={`mainservice-estimate-${data.slug}`}
                      className="inline-flex items-center gap-1.5 px-4 py-2.5 rounded-full bg-forest text-white text-sm font-bold hover:bg-[#1B5E20] transition-colors"
                    >
                      Get Free Estimate
                    </Link>
                  </div>
